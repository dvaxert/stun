...
TEST(SourceAddressTest, create_from_string) {
  ...
  ASSERT_EQ(attribute.Type(), AttributeType::SourceAddress);
}

//------------------------------------------------------------------------------

TEST(SourceAddressTest, create_from_int) {
...
TEST(SourceAddressTest, serialize_from_string) {
  auto testing_address = tests::GenerateRandomAddress();
  auto testing_port = tests::GenerateRandomValue<uint16_t>();
  ...
  std::vector<uint8_t> check = {0x00, 0x04, 0x00, 0x08, 0x00, 0x01};
...
TEST(SourceAddressTest, serialize_from_int) {
  auto testing_address = tests::GenerateRandomValue<uint32_t>();
  auto testing_port = tests::GenerateRandomValue<uint16_t>();
