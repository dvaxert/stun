...
TEST(ResponseAddressTest, create_from_string) {
  auto testing_address = tests::GenerateRandomAddress();
  auto testing_port = tests::GenerateRandomValue<uint16_t>();

  auto attribute = ResponseAddress(testing_address, testing_port);
  ...
  ASSERT_EQ(attribute.Type(), AttributeType::ResponseAddress);
}

//------------------------------------------------------------------------------

TEST(ResponseAddressTest, create_from_int) {
...
TEST(ResponseAddressTest, serialize_from_string) {
  auto testing_address = tests::GenerateRandomAddress();
  auto testing_port = tests::GenerateRandomValue<uint16_t>();
  ...
  std::vector<uint8_t> check = {0x00, 0x02, 0x00, 0x08, 0x00, 0x01};
...
TEST(ResponseAddressTest, serialize_from_int) {
  auto testing_address = tests::GenerateRandomValue<uint32_t>();
  auto testing_port = tests::GenerateRandomValue<uint16_t>();
